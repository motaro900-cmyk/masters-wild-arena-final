...
export class ShopScreen extends PIXI.Container {
    ...
    private renderItemGrid(startX: number, width: number) {
        ...
        filtered.forEach((item, i) => {
            ...
            // Основной фон карты
            const rarityColors: any = { COMMON: 0x333333, RARE: 0x1e3a8a, EPIC: 0x581c87, MYTHIC: 0x7f1d1d, LEGENDARY: 0x78350f };
            ...
            const name = new PIXI.Text({
                ...
                style: { fill: 0xFFFFFF, fontSize: 16, fontWeight: '900', align: 'center', wordWrap: true, wordWrapWidth: cellW - 20 }
            });
    ...
    private createBuyButton(w: number, h: number, color: number, label: string, onClick: () => void): PIXI.Container {
        const btn = new PIXI.Container();
        const bg = new PIXI.Graphics().roundRect(0, 0, w, h, 8).fill(color).stroke({ color: 0xffffff, width: 1, alpha: 0.3 });
    ...
    private showFlyText(parent: PIXI.Container, msg: string, color: number) {
        const txt = new PIXI.Text({ text: msg, style: { fill: color, fontSize: 24, fontWeight: 'bold', stroke: { color: 0x000000, width: 4 } } });
        txt.anchor.set(0.5);
    ...
    private createCloseButton(sw: number) {
        ...
        txt.anchor.set(0.5); txt.position.set(80, 25);
        btn.addChild(bg, txt);
