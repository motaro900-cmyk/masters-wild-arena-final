...
export class BattleScene extends PIXI.Container {
    ...
    constructor() {
        ...
        if (!store.consumeEnergy()) {
            ...
            txt.anchor.set(0.5); txt.position.set(640, 360);
            this.addChild(txt);
    ...
    private buildArena() {
        ...
        PIXI.Assets.load({ src: AssetsMap.BACKGROUNDS.BATTLE_ARENA }).then((tex) => {
    ...
    private buildFighters() {
        ...
        gsap.to(this.playerSprite.scale, { y: 0.97, x: 1.03, duration: 1.2, repeat: -1, yoyo: true, ease: "sine.inOut" });
        gsap.to(this.enemySprite.scale, { y: 0.97, x: 1.03, duration: 1.3, repeat: -1, yoyo: true, ease: "sine.inOut", delay: 0.2 });
        ...
        const createHpBar = (name: string, lvl: number, isPlayer: boolean, x: number, y: number) => {
            ...
            const bg = new PIXI.Graphics().roundRect(-120, 0, 240, 24, 12).fill(0x111111).stroke({ width: 2, color: 0x333333 });
    ...
    private buildHUD() {
        ...
        this.roundText = new PIXI.Text({
            text: 'РАУНД 1\n00:45',
            style: {
                fontFamily: 'Arial Black', fontSize: 36, fill: '#ffcc00',
                align: 'center', stroke: { color: '#331100', width: 6 }, dropShadow: { color: '#000000', alpha: 1, distance: 4, blur: 4, angle: Math.PI / 6 }
            }
        });
        ...
        energyTxt.anchor.set(0.5); energyTxt.position.set(960, 1040);
        ...
        gsap.to(source, {
            ...
            onComplete: () => {
                if (result.isDodge) {
                    ...
                    this.spawnDamagePopup(target.x + (Math.random()*40-20), target.y - 180, result.damage, result.isCrit);
                    
                    gsap.to(this.playerHpFill.scale, { x: Math.max(0, this.combatManager.playerHp / this.combatManager.playerMaxHp), duration: 0.3 });
                    gsap.to(this.enemyHpFill.scale, { x: Math.max(0, this.combatManager.enemyHp / this.combatManager.enemyMaxHp), duration: 0.3 });
                    this.checkCameraZoom();
                }

                gsap.to(source, { 
                    x: originalX, duration: 0.3, ease: 'power2.out',
                    onComplete: () => {
    ...
    private spawnDamagePopup(x: number, y: number, amount: number, isCrit: boolean) {
        const text = new PIXI.Text({
            ...
            style: {
                fontFamily: 'Impact', fontSize: isCrit ? 100 : 50, fill: isCrit ? '#ff2222' : '#ffffff',
                stroke: { color: '#000000', width: 8 }, dropShadow: { color: '#000000', alpha: 1, distance: 4, blur: 4, angle: Math.PI / 6 }
            }
        });
        text.anchor.set(0.5);
    ...
    private handleWin(winner: 'player'|'enemy') {
        ...
        const bg = new PIXI.Graphics().roundRect(-300, -200, 600, 450, 24).fill(0x1a1a2e).stroke({ width: 4, color: winner === 'player' ? 0xffcc00 : 0x555555 });
        modal.addChild(bg);
        ...
        const title = new PIXI.Text({
            ...
            style: {
                fontFamily: 'Arial Black', fontSize: 64, fill: winner === 'player' ? gradient : '#aaaaaa',
        ...
        const lootIcon = new PIXI.Text({
            text: '💰',
            style: { fontSize: 48, dropShadow: { color: '#000000', alpha: 1, distance: 3, blur: 4, angle: Math.PI / 6 } }
        });
        lootIcon.anchor.set(0.5); lootIcon.position.set(-80, 100);
        const lootText = new PIXI.Text({
            text: `+${earnedGold} ЗОЛОТА`,
            style: { fontSize: 24, fill: '#ffcc00', fontWeight: 'bold' }
        });
        lootText.anchor.set(0, 0.5); lootText.position.set(-30, 100);
        ...
        gsap.to(modal, { 
            y: 540, duration: 0.8, ease: 'back.out(1.2)', delay: 1,
            onComplete: () => {
