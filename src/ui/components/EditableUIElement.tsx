...
export const EditableUIElement: React.FC<Props> = ({ id, bgSrc, width, height }) => {
    ...
    return (
        <div ref={setNodeRef} style={style} className="group" {...(isEditMode && !isEditingText ? listeners : {})} {...(isEditMode && !isEditingText ? attributes : {})}>
            
            {/* Игровой Ассет: Подложка без CSS эффектов */}
            <img src={bgSrc} alt={`UI-${id}`} className="absolute inset-0 w-full h-full object-fill pointer-events-none select-none drop-shadow-lg" draggable={false} />
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                {isEditMode && isEditingText ? (
                    ...
                    <span 
                        ...
                        onDoubleClick={(e) => { if (isEditMode) { e.stopPropagation(); setIsEditingText(true); } }}
                    >
                        {elConfig.text}
                    </span>
                )}
            ...
            {isEditMode && <div className="absolute inset-0 border-2 border-dashed border-cyan-400 opacity-50 group-hover:opacity-100 pointer-events-none transition-opacity" />}
        </div>
    );
};
