...
export function makeDraggable(obj: PIXI.Container, name: string): void {
    ...
    if (savedPos) {
        try {
            ...
            if (parsed.alpha !== undefined) obj.alpha = parsed.alpha;
            if (parsed.tint !== undefined) (obj as any).tint = new PIXI.Color(parsed.tint).toNumber();
            if (parsed.zIndex !== undefined) obj.zIndex = parsed.zIndex;
    ...
    const getTintHex = (obj: PIXI.Container) => {
        const tint = (obj as any).tint;
        if (tint === undefined || tint === null) return '#ffffff';
        return '#' + (tint as number).toString(16).padStart(6, '0');
    };
    ...
    const handleUpdate = (e: any) => {
        if (e.detail.id === name && !isDragging) { // Защита от конфликта во время перетаскивания
            ...
            if (props.tint !== undefined) (obj as any).tint = new PIXI.Color(props.tint).toNumber();
