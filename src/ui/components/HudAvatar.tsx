...
export const HudAvatar: React.FC<HudAvatarProps> = ({ heroId }) => {
    ...
    useEffect(() => {
        ...
        const initPixi = async () => {
            ...
            const texture = AssetLoader.getInstance().getTexture(`hero_${heroId}`);
            ...
            gsap.to(sprite.scale, { y: sprite.scale.y * 1.05, x: sprite.scale.x * 0.95, duration: 1.5, repeat: -1, yoyo: true, ease: "sine.inOut" });
    ...
    return (
        <div ref={containerRef} className="w-16 h-16 bg-gradient-to-b from-[#1a365d] to-[#0a192f] rounded-full border-2 border-[#4a90e2] shadow-inner flex-shrink-0 overflow-hidden flex items-center justify-center" />
    );
};
