...
export const SideMenu: React.FC<{ activeTab: number; setActiveTab: (v: number) => void }> = ({ activeTab, setActiveTab }) => {
  ...
  const menuConfig = [
    ...
    <div ref={container} className="absolute left-4 top-[180px] flex flex-col z-50 pointer-events-none">
       <div className="mb-2.5 ml-4 flex items-center gap-2 pointer-events-auto">
          ...
          <span className="text-[8px] font-black text-green-500 uppercase italic tracking-widest leading-none">3 ОНЛАЙН</span>
       </div>
       {menuConfig.map((item, idx) => (
         <button 
           key={idx}
           onClick={() => { setActiveTab(idx); item.action(); }}
         ...
         >
           {activeTab === idx && <div className="absolute inset-0.5 border-2 border-amber-400/30 rounded-lg mix-blend-screen" />}
         </button>
       ))}
        </div>
    );
