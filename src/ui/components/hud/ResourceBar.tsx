        ...
        >
          ...
          {res.key === 'energy' && hoveredRes === 'energy' && (
            <div style={{
              ...
              <div style={{ color: '#f0c040', fontWeight: 900, fontSize: '13px', marginBottom: '8px', textAlign: 'center', textTransform: 'uppercase', fontFamily: "'Cinzel', serif" }}>
                Восстановление Энергии
              </div>
              <div className="flex flex-col gap-2">
                <div className="flex justify-between items-center text-[11px]">
                  <span style={{ color: '#a0a0a0' }}>До следующей:</span>
                  <span style={{ color: '#fff', fontWeight: 700 }}>{energy >= maxEnergy ? 'MAX' : timeLeft?.next}</span>
                </div>
                <div className="flex justify-between items-center text-[11px]">
                  <span style={{ color: '#a0a0a0' }}>До полной:</span>
                  <span style={{ color: '#fff', fontWeight: 700 }}>{energy >= maxEnergy ? '00:00' : timeLeft?.full}</span>
                </div>
                <div style={{ marginTop: '5px', height: '1px', background: 'rgba(240,192,64,0.2)' }} />
                <div style={{ color: '#8a7a6a', fontSize: '9px', textAlign: 'center', fontStyle: 'italic' }}>
                  {energy >= maxEnergy ? 'Энергия полностью восстановлена' : '1 ед. каждые 5 минут'}
                </div>
              </div>
              {/* Треугольничек */}
              <div style={{ position: 'absolute', top: '-6px', left: '50%', transform: 'translateX(-50%)', width: 0, height: 0, borderLeft: '6px solid transparent', borderRight: '6px solid transparent', borderBottom: '6px solid #f0c040' }} />
            </div>
          )}
