...
export const RewardPreviewModal: React.FC<{ item: any; onClose: () => void }> = ({ item, onClose }) => {
    ...
    if (item.type === 'WEAPON') {
        ...
        typeText = 'РАМКА АВАТАРА';
        description =
            'Уникальная рамка для вашего аватара. Позволяет украсить профиль и выделиться среди других игроков в таблице лидеров и чате.';
    } else if (item.type === 'AVATAR') {
        typeText = 'АВАТАР И ТИТУЛ';
        description =
            'Эксклюзивный аватар Короля Обезьян и титул "Король Арены". Настройте свой профиль, чтобы продемонстрировать свои достижения на арене.';
    } else if (item.type === 'TITLE') {
        typeText = 'ТИТУЛ';
        description =
            'Особый титул, отображающийся под вашим именем. Показывает ваш статус и заслуги в мире Диких Земель.';
    } else if (item.type === 'ENERGY') {
        typeText = 'ЭНЕРГИЯ';
        description =
            'Необходима для участия в битвах. Позволяет проводить больше сражений, зарабатывать опыт и получать награды на арене.';
    }
