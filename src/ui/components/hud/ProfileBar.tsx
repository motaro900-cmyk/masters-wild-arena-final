...
export const ProfileBar: React.FC<ProfileBarProps> = ({ onOpenProfile }) => {
  ...
  const nextRankTrophies = [100, 300, 500, 800, 1000, 1200, 1500, 1800, 2000, 2500, 3000, 5000].find(t => t > trophies) || 5000;
  const prevRankTrophies = [...[100, 300, 500, 800, 1000, 1200, 1500, 1800, 2000, 2500, 3000, 5000]].reverse().find(t => t <= trophies) || 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-6 relative z-20">
        <h2 className="text-[24px] font-bold leading-none mb-1.5"
          style={{
        ...
        <div className="flex items-center gap-2 mb-2.5">
          <svg width="11" height="11" viewBox="0 0 24 24" fill={rank.color} className="drop-shadow-[0_0_4px_rgba(255,255,255,0.3)]">
        ...
        <div className="flex items-center w-full gap-3">
          <div className="flex flex-col items-center justify-center">
            ...
            <span className="text-[22px] font-bold leading-none"
              style={{ fontFamily: "'Cinzel', serif", color: '#fdf5e6', textShadow: '0 0 10px rgba(255,255,255,0.3)' }}>{level}</span>
          </div>
          ...
          <div className="flex flex-col flex-1">
            <div className="relative h-[13px] w-full bg-black/80 rounded-[6px] border border-white/10 overflow-hidden shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)]">
              <motion.div
                animate={isLevelUpReady ? { boxShadow: ['0 0 0px #00d2ff', '0 0 20px #00d2ff', '0 0 0px #00d2ff'] } : {}}
            ...
            <div className="flex justify-between mt-1">
              <span className="text-[10px] font-black text-[#a0d8ff] tracking-tighter">
                {exp} <span className="opacity-30">/</span> {xpToNextLevel} <span className="text-[8px] opacity-50">EXP</span>
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="absolute right-10 top-0 bottom-0 flex flex-col items-end justify-center gap-5 z-20">
        <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-[12px] bg-black/60 border border-[#c8952a]/60 shadow-2xl">
          <svg width="18" height="18" viewBox="0 0 24 24" className="drop-shadow-[0_0_5px_#ffd700]">
            <path d="M18 2H6a1 1 0 00-1 1v3a4 4 0 004 4h1v3H8a1 1 0 00-1 1v3h10v-3a1 1 0 00-1-1h-2v-3h1a4 4 0 004-4V3a1 1 0 00-1-1zm-2 4V4h2v2a2 2 0 01-2 2h-1V6h1zM8 8a2 2 0 01-2-2V4h2v2h1v2H8z" fill="#ffd700" />
          </svg>
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl border text-[9px] font-black tracking-[0.3em] text-white shadow-xl"
