...
export const BattleScene: React.FC = () => {
    ...
    // Refs to track energy deduction and refund in case of errors
    const energyDeductedRef = useRef(false);
    const energyRefundedRef = useRef(false);

    // Предбоевой экран
    const currentBattleMode = useGameStore.getState().battleMode;
    const isRankedOrWarmup = currentBattleMode === 'RANKED' || currentBattleMode === 'WARMUP';
    const [showPreBattle, setShowPreBattle] = useState(!isRankedOrWarmup);
    const [battleStarted, setBattleStarted] = useState(isRankedOrWarmup);
    ...
    const enemyData = React.useMemo(() => {
        ...
        if ((battleMode === 'RANKED' || battleMode === 'WARMUP') && activeRankedOpponent) {
            return {
    ...
    useEffect(() => {
        ...
        if (store.battleMode === 'RANKED') {
            ...
            store.recordBattle();
            energyDeductedRef.current = true;
        }
    ...
    useEffect(() => {
        ...
        engine.onStateChange = (newState) => {
            try {
                ...
                if ((newState.playerHP <= 0 || newState.enemyHP <= 0) && !showResultRef.current) {
                    ...
                    let isVictory = newState.enemyHP <= 0;
                    const store = useGameStore.getState() as any;
                    ...
                    (async () => {
                        try {
                            ...
                            console.error('Ошибка в async-секции onStateChange:', innerErr);
                            refundEnergyIfDeducted(innerErr.message || 'Inner onStateChange error');
                        }
                    })();
                }
            } catch (err: any) {
                console.error('Ошибка в обработчике onStateChange:', err);
                refundEnergyIfDeducted(err.message || 'onStateChange error');
            }
        };
        ...
        engine.onCombatEvent = (event) => {
            try {
                ...
                console.error('Ошибка в обработчике onCombatEvent:', err);
                refundEnergyIfDeducted(err.message || 'onCombatEvent error');
            }
        };
        ...
        const run = async () => {
            ...
            if (containerRef.current) {
                try {
                    await engine.init(containerRef.current, selectedHeroId, rawEnemy.id, playerStats, enemyStats);
                } catch (err: any) {
                    console.error('[BattleScene] Критическая ошибка инициализации боя:', err);
                    refundEnergyIfDeducted(err.message || 'init error');
                }
            }
        };
        run();
                        ...
                        <PreBattleScreen
                            ...
                            enemyLevel={
                                battleMode === 'PVE' && activePveEnemy
                                    ...
                                    : (battleMode === 'RANKED' || battleMode === 'WARMUP') && activeRankedOpponent
