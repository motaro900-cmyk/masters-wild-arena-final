...
export const RightPanel: React.FC<RightPanelProps> = ({ onOpenWindow }) => {
    ...
    return (
        <div className="flex flex-col gap-4 items-end mr-2">
            ...
            {/* ЕЖЕДНЕВНЫЙ ПОДАРОК */}
            <div onClick={() => onOpenWindow('GIFT')} className="relative w-[270px] h-[110px] cursor-pointer transition-all hover:scale-105">
                ...
                <div className="absolute inset-0 flex items-center justify-center pt-2 pl-12">
                    <span className="font-header text-[15px] font-bold text-center leading-tight uppercase tracking-wide" style={whiteText}>
                        ПОДАРОК
                    </span>
                </div>
            </div>
            ...
            <div className="relative w-[340px] h-[220px]">
                <img src={AssetsMap.UI.PANEL_TASK} className="absolute inset-0 w-full h-full object-contain opacity-95" alt="" />
                <div className="relative z-10 pt-[35px] px-[45px]">
                    <h2 className="font-header text-center text-[20px] font-bold tracking-widest mb-4 uppercase" style={whiteText}>
                        КВЕСТЫ
                    </h2>
                    {[
                        ...
                        <div key={i} className="mb-4">
                            <div className="flex justify-between font-ui text-[12px] text-white font-bold mb-1 uppercase" style={whiteText}>
                                <span>{q.label}</span>
                                <span>{q.cur}/{q.max}</span>
                            </div>
                            <div className="w-full h-2 bg-black/60 rounded-full border border-amber-900/30 overflow-hidden shadow-inner">
                                <div className="h-full bg-gradient-to-r from-amber-600 to-amber-400" style={{ width: `${(q.cur/q.max)*100}%` }} />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className={`relative w-[380px] transition-all duration-300 ease-in-out ${isTasksExpanded ? 'h-[460px]' : 'h-[60px]'}`}>
                ...
                >
                    <h2 className="font-header text-center text-[24px] font-black tracking-widest uppercase" style={whiteText}>
                        ...
                        {dailyTasks.map((t, i) => (
                            ...
                            >
                                <div className="flex flex-col justify-center">
                                    ...
                                    <span className="font-ui text-[12px] font-black mt-1" style={{ color: '#FFD700', textShadow: '1px 1px 2px black' }}>
                                        💰 {t.g}
                                    </span>
                                </div>
