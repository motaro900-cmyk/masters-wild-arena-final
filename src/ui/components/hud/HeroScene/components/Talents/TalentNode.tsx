...
export const TalentNode = ({ talent, level, branchColor, isUnlocked, canAfford, onClick, onMouseEnter, onMouseLeave }: any) => {
    const isMax = level >= talent.max;
    return (
        <motion.div
            onMouseEnter={onMouseEnter}
            onMouseLeave={onMouseLeave}
            whileHover={isUnlocked && !isMax && canAfford ? { scale: 1.05, boxShadow: `0 0 35px ${branchColor}aa` } : {}}
            ...
            style={{
                ...
                position: 'relative',
                boxShadow: isMax ? `0 0 45px #f0c04066, inset 0 0 25px #f0c04022` : (isUnlocked ? `0 8px 25px rgba(0,0,0,0.4)` : 'none'),
                filter: isUnlocked ? 'none' : 'grayscale(1) brightness(0.4)',
                transition: 'all 0.25s ease-out'
            }}
        >
            {/* Увеличенная иконка таланта */}
            <div className={talent.iconClass} style={{ 
                width: '102px', 
                height: '102px', 
            ...
            <div style={{
                ...
                {level}<span style={{ opacity: 0.5, margin: '0 1px' }}>/</span>{talent.max}
            </div>
            ...
            {isMax && (
                <motion.div
                    ...
                    style={{ position: 'absolute', inset: '-6px', border: '2px dashed #f0c040', borderRadius: '28px', opacity: 0.3 }}
                />
            )}
        </motion.div>
    );
