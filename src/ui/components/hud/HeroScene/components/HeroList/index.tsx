...
function HeroCard({ hero, isOwned, isActive, isSelected, activeSkin, onClick }: any) {
    ...
    const border   = RARITY_BORDERS[activeRarity] || 'rgba(255,255,255,0.1)';
...
// ── STAT BOX ROW ─────────────────────────────────────────────────────────────
function StatBoxRow({ icon, label, value, color }: { icon: string; label: string; value: string | number; color: string }) {
    return (
        <div style={{
            display: 'flex',
            alignItems: 'center',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{
                    fontSize: '10.5px', color: 'rgba(255,255,255,0.4)',
...
function HeroDetailPanel({
    ...
    return (
        ...
        >
            <div style={{ padding: '24px 24px 16px', borderBottom: '1px solid rgba(255, 255, 255, 0.06)', background: 'rgba(0, 0, 0, 0.2)', flexShrink: 0 }}>
                <div style={{ color: '#fff', fontSize: '26px', fontWeight: 900, fontFamily: "'Cinzel', serif", letterSpacing: '0.8px', lineHeight: 1.15 }}>{displaySkin?.id === 'default' ? hero.name : displaySkin?.name}</div>
                <div style={{ marginTop: '6px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ color, fontSize: '9px', fontWeight: 900, letterSpacing: '1.5px', textTransform: 'uppercase', fontFamily: "'Nunito', sans-serif", background: `${color}15`, border: `1px solid ${color}33`, padding: '2px 7px', borderRadius: '6px' }}>{RARITY_LABELS[activeRarity]}</span>
                    <span style={{ color: 'rgba(255,255,255,0.15)', fontSize: '12px' }}>·</span>
                    <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '10.5px', fontFamily: "'Nunito', sans-serif", fontWeight: 700, letterSpacing: '0.5px' }}>{displaySkin?.id === 'default' ? hero.title : `Облик для ${hero.name}`}</span>
                </div>
            </div>

            <div className="custom-scrollbar" style={{ flex: 1, padding: '20px 24px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: `${roleInfo.bg}44`, border: `1px solid ${roleInfo.color}33`, padding: '10px 14px', borderRadius: '12px' }}>
                    <span style={{ fontSize: '18px' }}>{roleInfo.icon}</span>
                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                        <span style={{ color: roleInfo.color, fontSize: '11px', fontWeight: 800, fontFamily: "'Cinzel', serif", letterSpacing: '0.8px' }}>КЛАСС: {roleInfo.label}</span>
                        <span style={{ color: 'rgba(255, 255, 255, 0.35)', fontSize: '9px', fontFamily: "'Nunito', sans-serif", marginTop: '1px' }}>Уникальная роль в сражениях арены</span>
                    </div>
                </div>
                <div>
                    <div style={{ fontSize: '9.5px', fontWeight: 900, color: 'rgba(255,255,255,0.22)', fontFamily: "'Cinzel', serif", letterSpacing: '1.8px', marginBottom: '10px' }}>ХАРАКТЕРИСТИКИ</div>
                ...
                <div>
                    <div style={{ fontSize: '9.5px', fontWeight: 900, color: 'rgba(255,255,255,0.22)', fontFamily: "'Cinzel', serif", letterSpacing: '1.8px', marginBottom: '12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <span>ОБЛИКИ</span>
                        <span style={{ color: 'rgba(255,255,255,0.25)', fontWeight: 700, fontSize: '9px', fontFamily: "'Nunito', sans-serif" }}>{skins.filter(s => ownedSkins.includes(s.id) || s.source === 'default').length}/{skins.length} ОТКРЫТО</span>
                    </div>
                            ...
                            return (
                                <motion.div
                                    ...
                                    style={{
                                        ...
                                        border: isSelectedSkin ? `2px solid ${sc}` : '1.5px solid rgba(255,255,255,0.08)',
                                ...
                                >
                                    <img src={skin.image} style={{ height: '70%', width: 'auto', objectFit: 'contain', filter: skinOwned ? 'none' : 'grayscale(1) brightness(0.2)' }} alt={skin.name} draggable={false} />
                                    {isActiveSkin && (<div style={{ position: 'absolute', bottom: '6px', width: '6px', height: '6px', borderRadius: '50%', background: sc, boxShadow: `0 0 6px ${sc}` }} />)}
                                    {!skinOwned && (
                                        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(0, 0, 0, 0.4)' }}>
                                            ...
                                            <span style={{ fontSize: '9px', color: 'rgba(255,255,255,0.4)', fontFamily: "'Nunito', sans-serif", fontWeight: 800, textAlign: 'center', padding: '0 2px', textTransform: 'uppercase' }}>{SOURCE_ICONS[skin.source] || '🏆'} БП</span>
                                        </div>
                                    )}
                                </motion.div>
                            );
                    ...
                    <div style={{ marginTop: '10px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', padding: '8px 10px', border: '1px solid rgba(255,255,255,0.04)' }}>
                        <div style={{ color: '#fff', fontSize: '11.5px', fontWeight: 800, fontFamily: "'Cinzel', serif", letterSpacing: '0.3px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <span>{displaySkin?.name}</span>
                            <span style={{ fontSize: '8px', fontWeight: 900, color: rarityColors[displaySkin?.rarity] || '#fff', textTransform: 'uppercase', letterSpacing: '1px' }}>{RARITY_LABELS[displaySkin?.rarity]}</span>
                        </div>
                        <p style={{ margin: '4px 0 0', color: 'rgba(255,255,255,0.4)', fontSize: '9.5px', fontFamily: "'Nunito', sans-serif", lineHeight: 1.4 }}>{displaySkin?.description}</p>
                    </div>
                </div>
            </div>

            <div style={{ display: 'flex', gap: '12px', marginTop: 'auto', flexShrink: 0, padding: '16px 24px 24px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                {isOwned ? (
                    activeSkinId !== displaySkin.id ? (
                        (ownedSkins.includes(displaySkin.id) || displaySkin.source === 'default') ? (
                            <motion.button
                                ...
                                style={{
                                    ...
                                    letterSpacing: '1.5px', boxShadow: '0 4px 14px rgba(59,130,246,0.3)',
                            ...
                            <div
                                style={{
                                    ...
                                    borderRadius: '10px',
                                    color: '#ef4444', fontSize: '12px', fontWeight: 900,
                        ...
                        isActive ? (
                            <div style={{ flex: 1, padding: '12px 0', background: 'rgba(34, 197, 94, 0.12)', border: '1.5px solid rgba(34, 197, 94, 0.45)', borderRadius: '10px', color: '#4ade80', fontSize: '13px', fontWeight: 900, fontFamily: "'Cinzel', serif", letterSpacing: '1.5px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>✦ АКТИВЕН ДЛЯ БОЯ</div>
                        ) : (
                            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={onSelect} style={{ flex: 1, padding: '12px 0', background: 'linear-gradient(180deg, #f0c040 0%, #c8960a 100%)', border: 'none', borderRadius: '10px', color: '#1a0f00', fontSize: '13px', fontWeight: 900, cursor: 'pointer', fontFamily: "'Cinzel', serif", letterSpacing: '1.5px', boxShadow: '0 4px 14px rgba(240,192,64,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>ВЫБРАТЬ ГЕРОЯ</motion.button>
                        )
                    )
                ) : (
                    <>
                        <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '2px' }}>
                            <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: '8px', fontWeight: 800, fontFamily: "'Nunito', sans-serif", textTransform: 'uppercase', letterSpacing: '0.8px' }}>СТОИМОСТЬ</span>
                            <span style={{ color: hero.unlockType === 'diamonds' ? '#c084fc' : '#facc15', fontSize: '18px', fontWeight: 900, fontFamily: "'Nunito', sans-serif", display: 'flex', alignItems: 'center', gap: '4px' }}>{hero.unlockType === 'diamonds' ? '💎' : '🪙'} {hero.unlockCost}</span>
                        </div>
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={onBuy} style={{ flex: 1, padding: '12px 0', background: hero.unlockType === 'diamonds' ? 'linear-gradient(180deg, #a855f7 0%, #7c3aed 100%)' : 'linear-gradient(180deg, #f0c040 0%, #c8960a 100%)', border: 'none', borderRadius: '10px', color: hero.unlockType === 'diamonds' ? '#fff' : '#1a0f00', fontSize: '13px', fontWeight: 900, cursor: 'pointer', fontFamily: "'Cinzel', serif", letterSpacing: '1px', boxShadow: hero.unlockType === 'diamonds' ? '0 4px 14px rgba(168,85,247,0.35)' : '0 4px 14px rgba(240,192,64,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>🔓 РАЗБЛОКИРОВАТЬ</motion.button>
                    </>
                )}
            </div>
        </motion.div>
        ...
        <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        {(['ALL', 'OWNED'] as const).map(t => (
                            <motion.button
                                key={t}
                                whileTap={{ scale: 0.97 }}
                                onClick={() => { setSubTab(t); audioService.playSFX('SFX_CLICK'); }}
                                style={{
                    ...
                    <select
                        value={classFilter}
                        onChange={e => { setClassFilter(e.target.value); audioService.playSFX('SFX_CLICK'); }}
                        style={{
                            ...
                            letterSpacing: '0.5px', appearance: 'none',
                ...
                >
                    ...
                    <AnimatePresence>
                        {filteredHeroes.map((hero, idx) => {
                            ...
                            const activeSkinId = equippedSkins?.[hero.id] || defaultSkin.id;
                            const displaySkinId = (focusedId === hero.id && previewSkinId) ? previewSkinId : activeSkinId;
                    ...
                    {filteredHeroes.length === 0 && (
                        <div style={{
                            ...
                            <div style={{
                                color: 'rgba(255,255,255,0.2)', fontFamily: "'Cinzel', serif",
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        fontSize: '9.5px', fontFamily: "'Cinzel', serif",
                        fontWeight: 700, letterSpacing: '1px', marginBottom: '6px'
                    }}>
                        <span>КОЛЛЕКЦИЯ ГЕРОЕВ</span>
                        <span>{ownedHeroes.length} / {HEROES_DB.length}</span>
                    </div>
                    {/* Progress Bar */}
                    <div style={{
