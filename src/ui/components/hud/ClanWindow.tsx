                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                    <h2 style={{ margin: 0, fontFamily: "'Cinzel', serif", fontSize: '24px', color: colors.accent }}>
                        {view === 'BROWSE'
                            ...
                            : view === 'CREATE'
                              ? 'ОСНОВАНИЕ КЛАНА'
                              : `КЛАН: ${clanData?.name || newClanName}`}
                    </h2>
                    {view === 'DASHBOARD' && (
                        ...
                        >
                            ...
                            <div style={{ flex: 1 }}>
                                ...
                                <div style={{ display: 'flex', gap: '20px' }}>
                                    <StatBlock
                                        label="ОНЛАЙН"
                                        value={`${clanData?.onlineCount || 1}/${members.length}`}
                                    />
                                    <StatBlock
                                        label="ТРОФЕИ КЛАНА"
                                        value={
                                            clanData?.totalTrophies?.toLocaleString() || rating.toLocaleString() || '0'
                                        }
                                    />
                                </div>
                            </div>
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            <ActionButton
                                ...
                                color="#ef4444"
                                onClick={() => {
                                    setMembers((prev) => prev.filter((m) => m.name !== selectedMember.name));
...
const TabButton: React.FC<{ active: boolean; label: string; onClick: () => void; colors: any }> = ({
    active,
    label,
    onClick,
    colors,
...
const ClanCard: React.FC<{ clan: ClanData; onJoin: () => void; colors: any; playerTrophies: number }> = ({
    clan,
    onJoin,
    colors,
    playerTrophies,
