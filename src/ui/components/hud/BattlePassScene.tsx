...
interface Reward {
    level: number;
    free: { id: string, name: string, icon: string, amount?: number, type: 'GOLD' | 'ITEM' | 'CHEST' };
    premium: { id: string, name: string, icon: string, amount?: number, type: 'SKIN' | 'WEAPON' | 'GEMS' | 'CHEST' };
}
...
const DAILY_QUESTS: Quest[] = [
    { id: 'd1', title: 'Первая Кровь', description: 'Победите в 3 сражениях на Арене', progress: 1, target: 3, rewardXp: 200, icon: '⚔️' },
    { id: 'd2', title: 'Золотая Лихорадка', description: 'Заработайте 1000 золота в боях', progress: 450, target: 1000, rewardXp: 150, icon: '💰' },
    { id: 'd3', title: 'Алхимик', description: 'Используйте 5 любых зелий', progress: 2, target: 5, rewardXp: 100, icon: '🧪' },
];
...
export const BattlePassScene: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    const { bpLevel, bpExp, isPremium, claimReward, claimedRewards, setPremium, addItemToInventory, setEquippedWeapon } = useGameStore();
    ...
    return (
        ...
        >
            ...
            <div style={{ 
                height: '140px', padding: '0 80px', display: 'flex', alignItems: 'center',
                ...
                <div style={{ position: 'relative', width: '100px', height: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ 
                        position: 'absolute', inset: 0, background: 'linear-gradient(135deg, #f0c040 0%, #a88020 100%)',
                ...
                <div style={{ marginLeft: '40px', flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '30px' }}>
                        <div>
                            <h1 style={{ fontSize: '32px', margin: 0, fontFamily: "'Cinzel', serif", color: '#fff', letterSpacing: '2px', textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>БОЕВОЙ ПРОПУСК</h1>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginTop: '5px' }}>
                                <div style={{ width: '250px', height: '6px', background: 'rgba(255,255,255,0.1)', borderRadius: '3px', overflow: 'hidden' }}>
                                    <motion.div animate={{ width: `${progress}%` }} style={{ height: '100%', background: '#f0c040', boxShadow: '0 0 10px #f0c040' }} />
                                </div>
                                <span style={{ fontSize: '13px', fontWeight: 800, color: '#f0c040' }}>{bpExp} / {maxExp} XP</span>
                            </div>
                        </div>
                ...
                <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                    {!isPremium && (
                        <motion.button 
                            ...
                            whileHover={{ scale: 1.05, boxShadow: '0 0 30px rgba(240,192,64,0.4)' }} whileTap={{ scale: 0.95 }}
                            style={{
                                padding: '12px 30px', background: 'linear-gradient(180deg, #f0c040 0%, #a88020 100%)',
                                border: 'none', borderRadius: '10px', color: '#000', fontWeight: 900, fontSize: '15px',
                                cursor: 'pointer'
                            }}
                        >
                            КУПИТЬ ПРЕМИУМ
                    ...
                    <button onClick={onClose} style={{
                        width: '45px', height: '45px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)',
                        borderRadius: '8px', color: '#fff', fontSize: '20px', cursor: 'pointer'
                    }}>×</button>
                </div>
            </div>
            ...
            <div style={{ flex: 1, position: 'relative', zIndex: 5, overflow: 'hidden' }}>
                <AnimatePresence mode="wait">
                    {activeTab === 'REWARDS' ? (
                        <motion.div 
                            key="rewards" initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 50, opacity: 0 }}
                            style={{ height: '100%', display: 'flex', alignItems: 'center', padding: '0 100px', gap: '45px', overflowX: 'auto' }}
                            className="custom-scrollbar"
                        >
                            {BATTLE_PASS_REWARDS.map((reward) => (
                                <RewardColumn 
                                    key={reward.level} reward={reward} isUnlocked={bpLevel >= reward.level}
                                    isPremium={isPremium} claimedRewards={claimedRewards} 
                                    onClaim={handleClaim}
                                    onPreview={setSelectedReward} isMilestone={reward.level % 5 === 0}
                                />
                            ))}
                        </motion.div>
                    ) : (
                        <motion.div 
                            key="quests" initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -50, opacity: 0 }}
                            style={{ height: '100%', padding: '60px 100px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px' }}
                        >
            ...
            <AnimatePresence>
                ...
                {isPurchaseModalOpen && <PurchaseModal onClose={() => setIsPurchaseModalOpen(false)} onBuy={() => { setPremium(true); setIsPurchaseModalOpen(false); }} />}
            </AnimatePresence>
            ...
            <div style={{ 
                ...
                <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '14px', letterSpacing: '1px', fontWeight: 800 }}>ЗАРАБАТЫВАЙТЕ ОПЫТ В БОЯХ, ЧТОБЫ ОТКРЫВАТЬ НОВЫЕ УРОВНИ</p>
            </div>
        </motion.div>
    );
};
    ...
    return (
        <div style={{ 
            ...
            <div style={{ 
                ...
                <div style={{ position: 'absolute', width: '120%', height: '2px', background: isUnlocked ? '#f0c040' : 'rgba(255,255,255,0.1)', zIndex: 0 }} />
                <div style={{ 
                    ...
                    fontWeight: 900, color: isUnlocked ? '#000' : '#666', fontSize: isMilestone ? '28px' : '20px',
        ...
        <motion.div 
            ...
            style={{
                ...
                background: isMilestone 
                    ? (isPremiumCard ? 'linear-gradient(180deg, rgba(240,192,64,0.15) 0%, rgba(20,20,20,0.4) 100%)' : 'rgba(255,255,255,0.05)')
                    : (isPremiumCard ? 'rgba(240,192,64,0.1)' : 'rgba(255,255,255,0.03)'),
                borderRadius: '24px', 
                backdropFilter: 'blur(8px)',
                border: isMilestone 
                    ? `2px solid ${isPremiumCard ? '#f0c040' : 'rgba(255,255,255,0.4)'}` 
                    : `1px solid ${isPremiumCard ? 'rgba(240,192,64,0.4)' : 'rgba(255,255,255,0.15)'}`,
                padding: '25px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
                position: 'relative', overflow: 'hidden', cursor: 'pointer',
                filter: isUnlocked ? 'none' : 'grayscale(1) brightness(0.8) opacity(0.7)',
                opacity: isClaimed ? 0.4 : 1,
                boxShadow: isMilestone && isUnlocked ? `0 0 40px ${isPremiumCard ? 'rgba(240,192,64,0.2)' : 'rgba(255,255,255,0.1)'}` : 'none',
                transition: 'all 0.3s'
            }}
        >
            {isPremiumCard && <div style={{ position: 'absolute', top: '15px', right: '15px', fontSize: '24px', filter: 'drop-shadow(0 0 10px #f0c040)' }}>👑</div>}
            
            {isMilestone && (
                <div style={{ 
                    position: 'absolute', top: 0, left: 0, right: 0, height: '4px', 
                    background: isPremiumCard ? 'linear-gradient(90deg, transparent, #f0c040, transparent)' : 'linear-gradient(90deg, transparent, #fff, transparent)'
                }} />
            )}
            ...
            {!isUnlocked && isPremiumCard && !isClaimed && (
                <div style={{ 
                    ...
                    color: '#f0c040', fontWeight: 900, letterSpacing: '1px',
    ...
    return (
        ...
        >
            <motion.div 
                ...
                style={{
                    width: '600px', padding: '60px', background: 'rgba(30,30,30,0.95)',
                    borderRadius: '40px', border: '2px solid rgba(240,192,64,0.3)',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center',
                    boxShadow: '0 0 100px rgba(240,192,64,0.1)'
                }}
            >
                <div style={{ fontSize: '180px', marginBottom: '40px', filter: 'drop-shadow(0 20px 40px rgba(0,0,0,0.5))' }}>{item.icon}</div>
                <h2 style={{ fontSize: '42px', margin: 0, color: '#f0c040', fontFamily: "'Cinzel', serif" }}>{item.name}</h2>
                <p style={{ fontSize: '18px', color: 'rgba(255,255,255,0.6)', marginTop: '20px', lineHeight: '1.6' }}>
                    Эксклюзивная награда первого сезона. Улучшает ваши возможности и подчеркивает статус легендарного бойца арены.
                </p>
                <button onClick={onClose} style={{
                    marginTop: '40px', padding: '15px 50px', background: '#f0c040', border: 'none',
                    borderRadius: '12px', color: '#000', fontWeight: 900, fontSize: '18px', cursor: 'pointer'
                }}>ПОНЯТНО</button>
            </motion.div>
        </motion.div>
    );
...
const QuestSection: React.FC<{ title: string, quests: Quest[] }> = ({ title, quests }) => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
        <h3 style={{ fontSize: '24px', color: '#f0c040', fontFamily: "'Cinzel', serif", margin: '0 0 10px 0', letterSpacing: '2px' }}>{title}</h3>
        {quests.map(quest => (
            <motion.div 
                key={quest.id}
                whileHover={{ x: 10, background: 'rgba(255,255,255,0.05)' }}
                style={{
                    padding: '25px', background: 'rgba(255,255,255,0.03)', borderRadius: '20px',
                ...
                <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        <div style={{ flex: 1, height: '6px', background: 'rgba(0,0,0,0.3)', borderRadius: '3px', overflow: 'hidden' }}>
                            <div style={{ width: `${(quest.progress / quest.target) * 100}%`, height: '100%', background: '#f0c040' }} />
                        </div>
                        <span style={{ fontSize: '12px', fontWeight: 800 }}>{quest.progress} / {quest.target}</span>
                    </div>
                </div>
    ...
    return (
        ...
        >
            <motion.div 
                ...
                onClick={(e) => e.stopPropagation()}
                style={{
                    width: '900px', background: 'linear-gradient(135deg, #1a1a1a 0%, #0a0a0a 100%)',
                    borderRadius: '40px', border: '3px solid #f0c040', overflow: 'hidden',
                    display: 'flex', position: 'relative'
                }}
            >
                <div style={{ flex: 1, padding: '60px' }}>
                    <h2 style={{ fontSize: '48px', color: '#f0c040', fontFamily: "'Cinzel', serif", margin: 0 }}>ЗОЛОТОЙ ПРОПУСК</h2>
                    <p style={{ fontSize: '18px', color: 'rgba(255,255,255,0.5)', marginTop: '10px' }}>РАЗБЛОКИРУЙТЕ МАКСИМУМ ВОЗМОЖНОСТЕЙ</p>
                    
                    <ul style={{ listStyle: 'none', padding: 0, marginTop: '40px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        {['Эксклюзивная дорожка наград', 'Уникальный скин "Теневой Панда"', 'Множитель опыта +50%', 'Золотая рамка профиля'].map((text, i) => (
                            <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '15px', fontSize: '20px', fontWeight: 700 }}>
                    ...
                    <div style={{ marginTop: '60px', display: 'flex', alignItems: 'center', gap: '30px' }}>
                        <div style={{ fontSize: '32px', fontWeight: 900 }}>
                            <span style={{ fontSize: '20px', color: 'rgba(255,255,255,0.4)', textDecoration: 'line-through', marginRight: '10px' }}>1999</span>
                        ...
                        <motion.button 
                            ...
                            onClick={onBuy}
                            style={{
                                padding: '20px 60px', background: 'linear-gradient(180deg, #f0c040 0%, #a88020 100%)',
                                border: 'none', borderRadius: '15px', color: '#000', fontWeight: 900, fontSize: '22px',
                                cursor: 'pointer', boxShadow: '0 10px 40px rgba(240,192,64,0.4)'
                            }}
                        >
                            РАЗБЛОКИРОВАТЬ
                        </motion.button>
                ...
                <div style={{ width: '350px', background: 'rgba(240,192,64,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '200px' }}>
                    👑
                </div>
            </motion.div>
        </motion.div>
