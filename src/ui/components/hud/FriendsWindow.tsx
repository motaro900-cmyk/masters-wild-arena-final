...
export const FriendsWindow: React.FC<FriendsWindowProps> = () => {
    ...
    const [activeTab, setActiveTab] = useState<'ALL' | 'ONLINE' | 'REQUESTS' | 'REWARDS'>('ALL');
                ...
                <div style={{ display: 'flex', gap: '15px' }}>
                    <div style={{ position: 'relative', flex: 1 }}>
                        ...
                        <input 
                            ...
                            onKeyDown={(e) => {
                                if (e.key === 'Enter' && searchQuery.toUpperCase().startsWith('MW-')) {
                                    // Trigger global search or adding
                                    alert('Поиск игрока: ' + searchQuery);
                                }
                            }}
                            style={{
                    ...
                    <motion.button 
                        ...
                        onClick={() => {
                            ...
                            else if (searchQuery.toUpperCase().startsWith('MW-')) alert('Запрос отправлен ' + searchQuery);
                        }}
                        style={{ 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    ...
                    {activeTab === 'REWARDS' ? (
                        ...
                        <>
                                ...
                                >
                                    ...
                                    <div style={{ display: 'flex', gap: 8 }}>
                                        {activeTab === 'REQUESTS' ? (
                                            ...
                                            <>
                                                <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => sendGift(f.id)} style={{ width: 42, height: 42, background: f.giftSent ? 'rgba(255,255,255,0.05)' : 'rgba(240,192,64,0.1)', border: `1px solid ${f.giftSent ? colors.border : colors.accent}`, borderRadius: 12, cursor: f.giftSent ? 'default' : 'pointer', fontSize: 20, color: f.giftSent ? 'rgba(255,255,255,0.2)' : colors.accent, opacity: f.giftSent ? 0.5 : 1 }}>🎁</motion.button>
                            ...
                            {/* GLOBAL SEARCH RESULTS (MOCK) */}
                            {filteredFriends.length === 0 && searchQuery.toUpperCase().startsWith('MW-') && (
                                <motion.div 
                                    initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                                    style={{
                                        background: 'rgba(240,192,64,0.05)', border: `2px dashed ${colors.accent}`,
                                        borderRadius: 12, padding: '20px', display: 'flex', alignItems: 'center', gap: 15, marginTop: 10
                                    }}
                                >
                                    <div style={{ 
                                        width: 55, height: 55, background: '#1a1008', borderRadius: 10,
                                        border: `2px solid ${colors.accent}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24
                                    }}>
                                        👤
                                    </div>
                                    <div style={{ flex: 1 }}>
                                        <div style={{ fontFamily: "'Cinzel', serif", fontSize: 15, fontWeight: 700, color: colors.accent }}>ИГРОК НАЙДЕН</div>
                                        <div style={{ fontSize: 10, opacity: 0.7, fontWeight: 700 }}>ID: {searchQuery.toUpperCase()}</div>
                                        <div style={{ fontSize: 9, color: colors.accent, marginTop: 4, fontWeight: 900 }}>ГЛОБАЛЬНЫЙ ПОИСК</div>
                                    </div>
                                    <motion.button 
                                        whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                        onClick={() => {
                                            alert('Запрос в друзья отправлен игроку ' + searchQuery);
                                            setSearchQuery('');
                                        }}
                                        style={{
