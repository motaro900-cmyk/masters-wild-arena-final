...
export const BEASTS_DATABASE = [
    { id: 'moose', name: 'Лось', role: 'Танк', locked: false, condition: '', icon: '🫎', lore: 'Массивный страж северных лесов, покрытый древними рунами.' },
    { id: 'panda', name: 'Панда', role: 'Баланс', locked: false, condition: '', icon: '🐼', lore: 'Бывший охранник бамбукового склада. Теперь он наводит порядок в южных кварталах города. Его кулаки тяжелее его прошлого.' },
    { id: 'cat', name: 'Кот', role: 'Крит', locked: false, condition: '', icon: '🐱', lore: 'Ловкий карманник, знающий каждый переулок. Ни один кошелек не в безопасности.' },
    { id: 'tiger', name: 'Тигр', role: 'Урон', locked: true, condition: 'Достигните 10 уровня', icon: '🐯', lore: 'Мастер восточных единоборств, изгнанный из монастыря за излишнюю жестокость.' },
    { id: 'bear', name: 'Медведь', role: 'Танк', locked: true, condition: 'Победите босса Леса', icon: '🐻', lore: 'Непобежденный чемпион подпольных боев без правил.' },
    { id: 'lion', name: 'Лев', role: 'Баланс', locked: true, condition: 'Купите премиум-пропуск', icon: '🦁', lore: 'Король каменных джунглей, носящий золотую цепь как корону.' },
    { id: 'monkey', name: 'Обезьяна', role: 'Крит', locked: true, condition: 'Соберите 50 бананов', icon: '🐵', lore: 'Уличный трикстер с бейсбольной битой.' },
    { id: 'crocodile', name: 'Крокодил', role: 'Урон', locked: true, condition: 'Пройдите Болото', icon: '🐊', lore: 'Безжалостный рэкетир из городских доков.' },
    { id: 'rhino', name: 'Носорог', role: 'Танк', locked: true, condition: 'Соберите 100 жетонов', icon: '🦏', lore: 'Бронированный таран, способный пробить любую стену.' },
    { id: 'ram', name: 'Баран', role: 'Баланс', locked: true, condition: 'Выиграйте 5 турниров', icon: '🐏', lore: 'Упрямый боксер с невероятно крепким лбом.' },
    { id: 'panther', name: 'Пантера', role: 'Крит', locked: true, condition: 'Откройте в сундуке', icon: '🐆', lore: 'Теневой ассасин, действующий под покровом ночи.' },
    { id: 'boar', name: 'Кабан', role: 'Урон', locked: false, condition: '', icon: '🐗', lore: 'Неудержимый берсерк из грязных подворотен.' },
];
                ...
                return (
                    <div key={beast.id} 
                         ...
                         className={`relative rounded-3xl overflow-hidden border-4 transition-all duration-300 cursor-pointer flex flex-col items-center justify-between p-6 aspect-[4/5] select-none
                            ${isSelected ? 'border-yellow-500 shadow-[0_0_30px_rgba(234,179,8,0.4)] bg-gradient-to-b from-yellow-500/20 to-orange-600/20 scale-105 z-10' 
                            : beast.locked ? 'border-slate-800 bg-slate-900/50 grayscale hover:border-slate-600' 
                            : 'border-[#3e2b18] bg-[#1a120c] hover:border-yellow-500/50 hover:bg-[#2a1f16]'}`}
                    >
                        <span className={`text-9xl drop-shadow-xl transition-transform ${isSelected ? 'scale-110' : ''}`}>{beast.icon}</span>
                        <h4 className={`font-black text-xl tracking-widest uppercase mb-1 ${beast.locked ? 'text-slate-500' : 'text-white'}`}>{beast.name}</h4>
                        ...
                        {beast.locked ? (
                            <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center p-4 text-center backdrop-blur-[2px]">
                                ...
                                <span className="text-red-400 font-bold text-xs tracking-widest uppercase">{beast.condition}</span>
                            </div>
                        ) : (
                            <span className="text-yellow-500 font-black text-xs tracking-[0.2em] uppercase">{beast.role}</span>
                        )}
                    </div>
                );
            })}
...
const EquipSlot = ({ label, icon, item }: { label: string, icon: string, item: any }) => (
    <div className={`relative w-24 h-24 rounded-2xl flex flex-col items-center justify-center cursor-pointer transition-all border-2 select-none
        ${item ? 'bg-gradient-to-b from-yellow-500/20 to-orange-600/20 border-yellow-500 shadow-[0_0_20px_rgba(234,179,8,0.4)] hover:shadow-[0_0_30px_rgba(234,179,8,0.6)]' 
        : 'bg-[#0a0806] border-[#3e2b18] shadow-inner hover:border-[#d4b483]/50'}`}>
        <span className={`text-4xl transition-all ${item ? 'drop-shadow-[0_0_10px_rgba(255,255,255,0.8)] scale-110' : 'opacity-20 grayscale'}`}>
            {item ? '🗡️' : icon}
        </span>
...
const StatsView = ({ beastId: _beastId }: { beastId: string }) => {
    ...
    return (
        <div className="bg-[#16110d] border-2 border-[#3e2b18] rounded-2xl p-5 flex flex-col gap-3 shadow-inner select-none">
            <h4 className="text-[#a08b70] font-black text-xs tracking-widest uppercase mb-2 border-b border-[#3e2b18] pb-2 text-center">Характеристики</h4>
            {stats.map(s => (
                <div key={s.id} className="flex justify-between items-center bg-[#0a0806] p-2.5 rounded-xl border border-[#3e2b18]">
                    ...
                    <div className="flex items-baseline gap-1.5 font-mono">
                        ...
                        {s.bonus > 0 && <span className="text-green-400 font-bold text-xs drop-shadow-[0_0_5px_rgba(74,222,128,0.5)]">+{s.bonus}</span>}
                    </div>
                </div>
            ))}
        </div>
...
const EquipmentTab: React.FC = () => {
    ...
    React.useEffect(() => {
        ...
        const initPreview = async () => {
            ...
            container.x = 300; container.y = 700;
            ...
            const updateWeapon = async (id: string | null) => {
                if (!id) { weapon.visible = false; return; }
                const weaponFileMap: Record<string, string> = { '1': 'staff', '2': 'bow', '3': 'daggers', '4': 'axe', '8': 'moon_sword' };
                const file = weaponFileMap[id] || 'moon_sword';
                const tex = await PIXI.Assets.load(resolveAssetPath(`/assets/items/${file}.png`)).catch(() => null);
                if (tex) {
                    weapon.texture = tex;
                    weapon.visible = true;
                    weapon.x = 60; weapon.y = -460; weapon.rotation = 0.4;
        ...
        initPreview();
        return () => { if (app) app.destroy(true, { children: true }); };
        ...
        <div className="flex w-full h-full gap-4 relative overflow-hidden">
            ...
            <div className="flex-1 flex flex-col items-center justify-end pb-10 relative z-0">
                ...
                <div className="absolute bottom-4 text-center">
                    <h2 className="text-5xl font-black text-yellow-500 italic tracking-tighter uppercase drop-shadow-2xl">{beast.name}</h2>
                    <div className="flex items-center justify-center gap-2 mt-1">
                        <span className="w-3 h-3 bg-green-500 rounded-full animate-ping"></span>
                        <span className="text-green-400 font-black text-xs tracking-widest uppercase">Герой готов к бою</span>
                    </div>
                </div>
            </div>
                ...
                <div className="flex-1 bg-[#16110d]/90 backdrop-blur-md border-2 border-[#3e2b18] rounded-3xl p-6 flex flex-col shadow-2xl overflow-hidden">
                    ...
                    {rightPanelTab === 'inventory' && (
                        <div className="flex flex-col h-full">
                            <h4 className="text-[#a08b70] font-black text-sm tracking-widest uppercase mb-4 border-b border-[#3e2b18] pb-2">Твоё снаряжение</h4>
                            <div className="grid grid-cols-4 gap-3 overflow-y-auto custom-scrollbar pr-2 h-full content-start">
                                {inventory.map((item, i) => (
                                    <div key={item.id + i} onClick={() => equipWeapon(item.id)}
                                        ...
                                        {item.id === equippedWeaponId && <div className="absolute -top-1 -right-1 w-5 h-5 bg-green-500 rounded-full border-2 border-white flex items-center justify-center text-[10px]">✔</div>}
                                    </div>
                                ))}
                                {/* Пустые слоты для вида */}
                                {Array.from({length: Math.max(0, 20 - inventory.length)}).map((_, i) => (
                                    <div key={`empty-${i}`} className="aspect-square bg-[#0a0806]/50 border-2 border-[#3e2b18]/30 rounded-2xl" />
                                ))}
                            </div>
                        </div>
                    )}
                    ...
                    {rightPanelTab === 'lore' && (
                        <div className="flex flex-col h-full">
                            <h4 className="text-[#a08b70] font-black text-sm tracking-widest uppercase mb-4 border-b border-[#3e2b18] pb-2">История героя</h4>
...
const TalentTree: React.FC = () => {
    return (
        <div className="w-full h-full flex items-center justify-center relative bg-cover bg-center bg-blend-overlay bg-[#1a120c]/80 rounded-3xl overflow-hidden border-2 border-[#3e2b18] shadow-inner select-none" style={{ backgroundImage: `url(${AssetsMap.BACKGROUNDS.MAIN_MENU})` }}>
            {/* SVG Линии связи */}
            <svg className="absolute inset-0 w-full h-full pointer-events-none">
                <line x1="50%" y1="80%" x2="50%" y2="50%" stroke="#5e4125" strokeWidth="6" strokeDasharray="10 10" />
                <line x1="50%" y1="50%" x2="30%" y2="25%" stroke="#facc15" strokeWidth="8" className="drop-shadow-[0_0_15px_rgba(250,204,21,0.8)]" />
            ...
            <div className="absolute bottom-[15%] left-1/2 -translate-x-1/2 flex flex-col items-center gap-3">
                <button className="w-20 h-20 rounded-full border-4 border-yellow-500 bg-gradient-to-b from-yellow-400 to-orange-600 shadow-[0_0_30px_rgba(234,179,8,0.6)] flex items-center justify-center text-3xl hover:scale-110 active:scale-95 transition-all">🔥</button>
                <span className="text-yellow-500 font-black text-xs uppercase tracking-widest bg-black/80 px-3 py-1 rounded-md border border-yellow-500/30">Основа</span>
            </div>
            ...
            <div className="absolute top-[50%] left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-3">
                ...
                <span className="text-yellow-500 font-black text-xs uppercase tracking-widest bg-black/80 px-3 py-1 rounded-md border border-yellow-500/30">Сила двора</span>
            </div>

            <div className="absolute top-[25%] left-[30%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-3">
                <button className="w-20 h-20 rounded-full border-4 border-yellow-500 bg-gradient-to-b from-yellow-400 to-orange-600 shadow-[0_0_30px_rgba(234,179,8,0.6)] flex items-center justify-center text-3xl hover:scale-110 active:scale-95 transition-all">⚡</button>
                <span className="text-yellow-500 font-black text-xs uppercase tracking-widest bg-black/80 px-3 py-1 rounded-md border border-yellow-500/30">Ловкость</span>
            </div>

            <div className="absolute top-[25%] left-[70%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center gap-3">
                <button className="w-20 h-20 rounded-full border-4 border-[#5e4125] bg-[#1a120c] flex items-center justify-center text-3xl hover:border-[#d4b483] transition-all grayscale opacity-50 hover:grayscale-0 hover:opacity-100 cursor-pointer">🛡️</button>
                <span className="text-[#a08b70] font-black text-xs uppercase tracking-widest bg-black/80 px-3 py-1 rounded-md border border-[#3e2b18]">Защита</span>
            </div>
        </div>
    );
};
        ...
        <div className="w-full h-full pointer-events-none flex items-center justify-center font-sans">
            ...
            >
                ...
                <div 
                    ...
                    style={{ clipPath: 'polygon(38px 0%, calc(100% - 38px) 0%, 100% 38px, 100% calc(100% - 38px), calc(100% - 38px) 100%, 38px 100%, 0% calc(100% - 38px), 0% 38px)' }}
                >
                    ...
                    <div className="flex items-center justify-between px-14 py-6 border-b border-[#3e2b18] bg-gradient-to-b from-[#1a120c] to-transparent shrink-0">
                        <div className="flex items-center gap-6 select-none">
                            <div className="w-16 h-16 bg-yellow-500/10 border border-yellow-500/30 rounded-2xl flex items-center justify-center text-3xl shadow-[0_0_15px_rgba(250,204,21,0.2)]">🐾</div>
                            <div className="flex flex-col">
                                <h2 className="text-4xl font-black text-white italic tracking-widest uppercase drop-shadow-md">Звери</h2>
                                <span className="text-yellow-500/80 font-bold text-xs tracking-[0.4em] uppercase">Отряд чемпионов</span>
                            </div>
                        </div>
                        ...
                        {/* Close Button */}
                        <button onClick={goToMainMenu} className="w-14 h-14 bg-[#2a1f16] border-2 border-[#5e4125] hover:border-red-500 hover:bg-red-500/20 rounded-2xl flex items-center justify-center text-white/50 hover:text-red-400 transition-all shadow-lg active:scale-95 pointer-events-auto cursor-pointer">
                    ...
                    <div className="flex-1 p-8 overflow-hidden relative">
                        {activeTab === 'collection' && <BeastGrid selectedId={selectedBeastId} onSelect={setSelectedBeastId} />}
