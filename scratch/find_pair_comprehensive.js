...
function createCombatant(heroId, role, stats, level, varkanActiveDmg = 3.25, varkanMarkBonus = 0.2) {
    ...
    return {
        ...
        hunterStacks: 0,
        attackCounter: 0,
        activeDmg: heroId === 'panda' ? 2.5 : heroId === 'raccoon' ? 3.5 : heroId === 'tiger_warrior' ? varkanActiveDmg : heroId === 'lion_knight' ? 2.4 : 1.8,
...
function simulateCombat(p1, p2) {
    ...
    const executeAttack = (isP1) => {
        ...
        if (att.id === 'tiger_warrior') {
            ...
            if (hasMarkAtStart) {
                mult += att.varkanMarkBonus * factor;
            }
        }
        damage *= mult;
...
// We will scan:
// - Rikki strength (2 to 14, stamina is 29 - strength, agility 25)
// - Varkan strength (12 to 20, stamina is 32 - strength, agility 24)
// - Varkan activeDmg (2.6, 2.8, 3.0, 3.25, 3.5, 3.8, 4.0)
// - Varkan markBonus (0.2, 0.4, 0.5, 0.6, 0.7)

const activeDmgs = [2.6, 2.8, 3.0, 3.25, 3.5, 3.8, 4.0];
const markBonuses = [0.2, 0.4, 0.5, 0.6, 0.7];

console.log("Comprehensive search started...");

for (let racStr = 2; racStr <= 14; racStr++) {
    const racSta = 29 - racStr;
    const racStats = { strength: racStr, agility: 25, stamina: racSta };

    for (let tigStr = 12; tigStr <= 20; tigStr++) {
        const tigSta = 32 - tigStr;
        const tigStats = { strength: tigStr, agility: 24, stamina: tigSta };

        for (const vAct of activeDmgs) {
            for (const vMark of markBonuses) {
                
                // Quick test of raccoon vs tiger_warrior
                let racWins = 0;
                const runs = 300; // smaller run size for speed
                for (let r = 0; r < runs; r++) {
                    const h1IsP1 = (r % 2 === 0);
                    const p1 = h1IsP1 ? createCombatant('raccoon', 'ASSASSIN', racStats, 1, vAct, vMark) : createCombatant('tiger_warrior', 'ASSASSIN', tigStats, 1, vAct, vMark);
                    const p2 = h1IsP1 ? createCombatant('tiger_warrior', 'ASSASSIN', tigStats, 1, vAct, vMark) : createCombatant('raccoon', 'ASSASSIN', racStats, 1, vAct, vMark);
                ...
                if (tigWR_vs_rac >= 0.60 && tigWR_vs_rac <= 0.70) {
                    ...
                    for (const otherId in HEROES_DB) {
                        ...
                        for (let r = 0; r < runs; r++) {
                            const h1IsP1 = (r % 2 === 0);
                            const p1 = h1IsP1 ? createCombatant('raccoon', 'ASSASSIN', racStats, 1, vAct, vMark) : createCombatant(other.id, other.role, other.stats, 1, vAct, vMark);
                            const p2 = h1IsP1 ? createCombatant(other.id, other.role, other.stats, 1, vAct, vMark) : createCombatant('raccoon', 'ASSASSIN', racStats, 1, vAct, vMark);
                            const win = simulateCombat(p1, p2);
                    ...
                    const racOverall = (totalRacWR + 0.5) / 5;

                    if (racOverall >= 0.45 && racOverall <= 0.55) {
                        ...
                        for (const otherId in HEROES_DB) {
                            ...
                            for (let r = 0; r < runs; r++) {
                                const h1IsP1 = (r % 2 === 0);
                                const p1 = h1IsP1 ? createCombatant('tiger_warrior', 'ASSASSIN', tigStats, 1, vAct, vMark) : createCombatant(other.id, other.role, other.stats, 1, vAct, vMark);
                                const p2 = h1IsP1 ? createCombatant(other.id, other.role, other.stats, 1, vAct, vMark) : createCombatant('tiger_warrior', 'ASSASSIN', tigStats, 1, vAct, vMark);
                                const win = simulateCombat(p1, p2);
                                if (h1IsP1) otherWins += win;
                                else otherWins += (1 - win);
                            }
                        ...
                        if (tigOverall >= 0.45 && tigOverall <= 0.55) {
                            console.log(`SOLUTION FOUND:`);
                            console.log(`  Rikki stats: strength=${racStr}, agility=25, stamina=${racSta} (Overall WR: ${(racOverall*100).toFixed(1)}%)`);
                            console.log(`  Varkan stats: strength=${tigStr}, agility=24, stamina=${tigSta} (Overall WR: ${(tigOverall*100).toFixed(1)}%)`);
                            console.log(`  Varkan activeDmg: ${vAct}, shadow mark bonus: ${vMark}`);
                            console.log(`  Varkan vs Rikki WR: ${(tigWR_vs_rac*100).toFixed(1)}%`);
                        }
                    }
                }
            }
...
console.log("Comprehensive search ended.");
