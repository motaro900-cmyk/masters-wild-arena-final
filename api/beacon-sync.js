 ...
 * /api/beacon-sync
 *
 * Receives a critical game state snapshot via navigator.sendBeacon() or
 * keepalive fetch when the user closes the VK Mini App.
 *
 * VK blocks standard beforeunload/unload events per Permissions Policy,
 * so this endpoint handles guaranteed data persistence on page close.
 *
 * Uses Firebase REST API (no Admin SDK required) with service account auth.
 */
